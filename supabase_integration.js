...
function tgSend(msg){
  ...
  fetch('/api/notify', {
    ...
    /* Fallback: direct Telegram (works if CORS allows) */
...
function tgNotifyRecarga(username, monto, metodo, txid, waNum){
  ...
  fetch('/api/notify', {
    ...
    body: JSON.stringify({
      message: msg,
      buttons: [
        { text: '\u2705 ACEPTAR $' + montoNum.toLocaleString('es-MX'), data: 'accept:' + username + ':' + montoNum },
        { text: '\u274C RECHAZAR', data: 'reject:' + username + ':' + montoNum }
      ]
    })
  }).catch(function(){});
}
