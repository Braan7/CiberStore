...
function addSpend(amt, descripcion){
  ...
  var desc = descripcion || 'Compra en CiberStore';
  var ordMatch = desc.match(/#(\d+)/);
  var ordNum = ordMatch ? ordMatch[1] : '?';
  tgNotifyPurchase(authSession.username, desc.replace(' - Pedido #'+ordNum,''), amt, ordNum);
  if(typeof updateAuthUI === 'function') updateAuthUI();
