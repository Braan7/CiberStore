...
function doRegister(){
  ...
  var whatsapp  = ((document.getElementById('reg-whatsapp') || {}).value || '').trim().replace(/\D/g,'');
  ...
  if(!whatsapp || whatsapp.length < 8)      { showAuthMsg('reg-err', 'Ingresa tu WhatsApp'); return; }
  ...
  sbGetByUsername(username).then(function(existing){
    ...
    return sbCreateProfile({
      ...
      if(!profile){
        showAuthMsg('reg-err', 'Error al crear cuenta. Intenta de nuevo.');
        if(ok) ok.style.display = 'none';
        return;
      }
      showAuthMsg('reg-ok', '\u2714 Cuenta creada! Entrando...', true);
      setTimeout(function(){ loginWithProfile(profile); }, 800);
    });
  }).catch(function(e){
    showAuthMsg('reg-err', 'Error de conexion: ' + (e.message || 'intenta de nuevo'));
    if(ok) ok.style.display = 'none';
  });
}
...
function doLogin(){
  ...
  sbGetByUsername(username).then(function(u){
    ...
    showAuthMsg('login-err', 'Error de conexion: ' + (e.message || 'intenta de nuevo'));
  });
}
...
function renderAdminUsers(){
  ...
  sb.get('profiles', qs).then(function(users){
    ...
    list.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--muted)">Error: ' + (e.message || 'intenta de nuevo') + '</div>';
  });
}

function renderAdminStats2(users){
  if(!users){
    sb.get('profiles', 'select=saldo').then(function(u){ renderAdminStats2(u); }).catch(function(){});
    return;
  }
...
(function initSB(){
  ...
  if(saved && saved.id){
    sbGetById(saved.id).then(function(u){
      ...
      if(typeof updateAuthUI === 'function') setTimeout(updateAuthUI, 50);
