...
function doLogin(){
  ...
  /* Timeout fallback - if no response in 10s show error */
  var loginTimeout = setTimeout(function(){
    showAuthMsg('login-err', 'Sin respuesta de Supabase. Verifica tu conexion.');
  }, 10000);
  ...
  sbGetByUsername(username).then(function(u){
    ...
    if(msg.indexOf('does not exist') > -1 || msg.indexOf('relation') > -1){
      msg = 'Tabla no existe. Corre setup_supabase.sql en Supabase primero.';
    }
    if(msg.indexOf('allowlist') > -1){
      msg = 'Error de configuracion Supabase (allowlist). Contacta soporte.';
    }
...
(function initSB(){
  ...
  if(saved && saved.id){
    /* Show page immediately - verify session in background */
    ...
    /* Verify + refresh from DB in background */
    setTimeout(function(){
      sbGetById(saved.id).then(function(u){
        ...
        if(typeof updateAuthUI === 'function') updateAuthUI();
        if(typeof renderPerfil === 'function') renderPerfil();
      }).catch(function(){ /* keep local session on network error */ });
    }, 500);
  } else {
    /* No session - show auth modal after page renders */
    setTimeout(function(){
      if(typeof showAuthModal === 'function') showAuthModal();
    }, 200);
  }
