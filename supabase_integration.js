...
function tgNotifyRegister(profile){
  ...
  try {
    if(typeof sb !== 'undefined' && sb.count){
      sb.count('profiles').then(function(total){
        armarYenviar(total || null);
      }).catch(function(){ armarYenviar(null); });
    } else {
      // Respaldo si sb.count no esta disponible (version vieja de supabase.js)
...
function tgNotifyRecarga(username, monto, metodo, txid, waNum){
  var montoNum = parseFloat(monto||0);
  var msg = '\uD83D\uDCB3 <b>Solicitud de recarga - CiberStore</b>\n\n'
    + '\uD83D\uDC64 Usuario: <b>' + username + '</b>\n'
    + '\uD83D\uDCF1 WhatsApp: +' + (waNum || '-') + '\n'
    + '\uD83D\uDCB0 Monto: <b>$' + montoNum.toLocaleString('es-MX') + ' MX</b>\n'
    + '\uD83C\uDFE6 Metodo: ' + (metodo || '-') + '\n'
    + '\uD83D\uDD11 TXID: ' + (txid || '-') + '\n'
    + '\uD83D\uDD50 ' + new Date().toLocaleString('es-MX') + '\n\n'
    + '\u26A0\uFE0F <i>Toca un boton para aprobar o rechazar</i>';
  /* Send directly to Telegram with inline buttons */
  fetch('https://api.telegram.org/bot' + TG_TOKEN + '/sendMessage', {
    ...
    body: JSON.stringify({
      ...
      reply_markup: {
        inline_keyboard: [[
          { text: '\u2705 ACEPTAR $' + montoNum.toLocaleString('es-MX'), callback_data: 'accept:' + username + ':' + montoNum },
          { text: '\u274C RECHAZAR', callback_data: 'reject:' + username + ':' + montoNum }
        ]]
      }
    })
  }).catch(function(){});
...
function sbGetByUsername(username){
  return sb.get('profiles', 'username=eq.' + encodeURIComponent(username) + '&limit=1&select=id,username,password_hash,role,saldo,nombre,whatsapp,ref_code,banned,created_at')
           .then(function(r){
             if(!r || !Array.isArray(r)) return null;
             var user = r[0] || null;
             /* Cache successful user lookup */
             if(user){
               try{ localStorage.setItem('cs_ucache_'+username, JSON.stringify(user)); }catch(e){}
             }
             return user;
           });
}
...
function sbGetByUsernameWithCache(username){
  ...
  return sbGetByUsername(username).catch(function(){
    try{
      var cached = localStorage.getItem('cs_ucache_'+username);
...
/* ── LOGIN ──────────────────────────────────────────────────── */
/* ================================================================
   LOGIN SECURITY — Rate limiting + Remember Me
================================================================ */
function getLoginAttempts(){
  try {
    var d = JSON.parse(localStorage.getItem('cs_login_attempts')||'{}');
    return d;
  } catch(e){ return {}; }
}
...
function recordFailedAttempt(username){
  ...
  if(!d[username]) d[username] = {count:0, lastFail:0};
  d[username].count++;
  d[username].lastFail = Date.now();
  localStorage.setItem('cs_login_attempts', JSON.stringify(d));
  return d[username].count;
}
...
function isBlocked(username){
  var d = getLoginAttempts();
  var u = d[username];
  if(!u || u.count < 3) return false;
  var elapsed = Date.now() - u.lastFail;
  var blockTime = 5 * 60 * 1000; /* 5 minutes */
  if(elapsed < blockTime){
    return Math.ceil((blockTime - elapsed) / 1000); /* seconds left */
  }
  clearAttempts(username);
  return false;
}

function updateAttemptsUI(username){
  var info = document.getElementById('login-attempts-info');
  if(!info) return;
  var d = getLoginAttempts();
  var u = d[username];
  if(!u) { info.style.display='none'; return; }
  var blocked = isBlocked(username);
  if(blocked){
    var mins = Math.ceil(blocked/60);
    info.textContent = '\uD83D\uDD12 Bloqueado '+mins+' min';
    info.style.display='block';
  } else if(u.count > 0){
    info.textContent = (3-u.count)+' intentos restantes';
    info.style.display='block';
  }
}

/* Show block timer countdown */
function startBlockCountdown(username){
  var btn = document.getElementById('btn-do-login');
  var interval = setInterval(function(){
    var blocked = isBlocked(username);
    if(!blocked){
      clearInterval(interval);
      showAuthMsg('login-err','Ya puedes intentar de nuevo.', false,'rgba(0,230,118,.7)');
      if(btn){ btn.disabled=false; btn.style.opacity='1'; }
      var info=document.getElementById('login-attempts-info');
      if(info) info.style.display='none';
      return;
    }
    var mins = Math.floor(blocked/60);
    var secs = blocked % 60;
    showAuthMsg('login-err',
      '\uD83D\uDD12 Demasiados intentos. Espera '+mins+'m '+secs+'s',
      false,'rgba(255,80,80,.7)');
    updateAttemptsUI(username);
  }, 1000);
}

/* Remember Me — save/restore username */
function applyRememberMe(){
  var saved = localStorage.getItem('cs_remember_user');
  if(saved){
...
function saveRememberMe(username){
  var cb = document.getElementById('login-remember');
  if(cb && cb.checked){
    localStorage.setItem('cs_remember_user', username);
  } else {
    localStorage.removeItem('cs_remember_user');
  }
}
...
function doLogin(){
  ...
  if(blocked){
    var mins = Math.floor(blocked/60);
    var secs = blocked % 60;
    showAuthMsg('login-err','\uD83D\uDD12 Bloqueado. Espera '+mins+'m '+secs+'s', false,'rgba(255,80,80,.7)');
    startBlockCountdown(username);
  ...
  sbGetByUsernameWithCache(username).then(function(u){
    if(err) err.style.display='none';
    if(!u){
      recordFailedAttempt(username);
      updateAttemptsUI(username);
      showAuthMsg('login-err','Usuario no encontrado');
      return;
    }
    ...
    if(!ok){
      var oh=0;
      for(var i=0;i<pass.length;i++){var c=pass.charCodeAt(i);oh=((oh<<5)-oh)+c;oh|=0;}
      if(u.password_hash===oh.toString(36)){ sbUpdateProfile(u.id,{password_hash:hashPass(pass)}); ok=true; }
    }
    if(!ok){
      var attempts = recordFailedAttempt(username);
      updateAttemptsUI(username);
      if(attempts >= 3){
        showAuthMsg('login-err','\uD83D\uDD12 Demasiados intentos. Bloqueado 5 minutos.', false,'rgba(255,80,80,.7)');
        startBlockCountdown(username);
        if(btn){ btn.disabled=true; btn.style.opacity='.5'; }
      } else {
        showAuthMsg('login-err','Contrasena incorrecta. '+(3-attempts)+' intentos restantes.');
      }
      return;
    }
    clearAttempts(username);
...
function loginWithProfile(profile){
  ...
  if(profile.role === 'admin'){
    adminAuthed = true;
    setTimeout(function(){
      if(typeof openFullAdmin === 'function') openFullAdmin();
    }, 400);
...
(function initSB(){
  loadPromos();
  setTimeout(applyRememberMe, 300);
...
================================================================ */
var REF_BONUS = 2; /* MXN bonus per referral */

function generateRefCode(username){
  /* Simple code: first 3 chars of username + 4 random digits */
  var base = username.replace(/[^a-z0-9]/gi,'').substring(0,4).toUpperCase();
...
function loadRefData(){
  ...
  var earnEl  = document.getElementById('ref-earned');

  /* Show ref code */
  var code = authSession.ref_code || '';
  if(codeEl) codeEl.textContent = code || '-';

  /* Count referrals - people who used this code */
  if(!code) return;
  sb.get('profiles','ref_code=eq.'+encodeURIComponent(code)+'&select=id')
    .then(function(rows){
      var count = rows ? rows.length : 0;
      if(countEl) countEl.textContent = count;
      if(earnEl)  earnEl.textContent  = '$' + (count * REF_BONUS).toLocaleString('es-MX') + ' MX';
    }).catch(function(){});
}

function copyRefCode(){
  var code = (document.getElementById('ref-code-display')||{}).textContent || '';
  if(!code || code==='-'){ showToast('Inicia sesion para ver tu codigo'); return; }
...
function shareRefCode(){
  ...
  var code = authSession.ref_code || '';
  if(!code){ showToast('Sin codigo asignado'); return; }
  var msg = 'Registrate en CiberStore y usa mi codigo *'+code+'* al registrarte. '
    +'Ambos recibimos $'+REF_BONUS+' MX de saldo gratis! \nCiberstore.lat';
  var url = 'https://wa.me/?text='+encodeURIComponent(msg);
  var w = window.open(url,'_blank');
  if(!w) window.location.href = url;
}

/* Apply referral bonus when someone registers with a ref code */
function applyRefBonus(refCode, newUserId){
  if(!refCode) return;
  /* Find referrer */
  sb.get('profiles','select=id,username,saldo&limit=1')
    .then(function(allUsers){
      /* Actually query by matching ref_code pattern */
      /* refCode is stored in profiles.ref_code of the referrer */
      sb.get('profiles','select=id,username,saldo').then(function(users){
        if(!users||!Array.isArray(users)) return;
        var referrer = null;
        for(var i=0;i<users.length;i++){
          if(users[i].ref_code && users[i].ref_code.toUpperCase()===refCode.toUpperCase()){
            referrer=users[i]; break;
          }
        }
        if(!referrer || referrer.id===newUserId) return;
        /* Give bonus to referrer */
        var newSaldo = (referrer.saldo||0) + REF_BONUS;
        sbUpdateProfile(referrer.id, {saldo: newSaldo});
        sbAddMovimiento(referrer.id, 'credito', REF_BONUS, 'Bono referido: +$2 MX por invitacion');
        /* Give bonus to new user */
        sbUpdateProfile(newUserId, {saldo: REF_BONUS});
        sbAddMovimiento(newUserId, 'credito', REF_BONUS, 'Bono bienvenida por usar codigo de referido');
      });
    }).catch(function(){});
}
...
function sendDailyReport(){
  ...
  sb.get('movimientos_saldo','tipo=eq.compra&created_at=gte.'+since+'&select=monto,descripcion,user_id')
    .then(function(movs){
      var totalVentas = 0;
      var numPedidos  = 0;
      if(movs&&Array.isArray(movs)){
        movs.forEach(function(m){ totalVentas+=(m.monto||0); numPedidos++; });
      }
      sb.get('profiles','created_at=gte.'+since+'&select=id').then(function(newUsers){
        var nuevoReg = newUsers ? newUsers.length : 0;
        var fecha = today.toLocaleDateString('es-MX',{weekday:'long',day:'2-digit',month:'long'});
        var msg = '\uD83D\uDCCA <b>Reporte Diario - CiberStore</b>\n'
          + '\uD83D\uDCC5 ' + fecha + '\n\n'
          + '\uD83D\uDCB0 Ventas del dia: <b>$' + totalVentas.toLocaleString('es-MX') + ' MX</b>\n'
          + '\uD83D\uDED2 Pedidos: <b>' + numPedidos + '</b>\n'
          + '\uD83D\uDC65 Nuevos registros: <b>' + nuevoReg + '</b>\n\n'
          + (numPedidos===0 ? '\uD83D\uDE34 Sin pedidos ayer.' : '\uD83D\uDD25 Buen dia de ventas!');
        /* Send via proxy */
        fetch('/api/notify', {
          method:'POST',
          headers:{'Content-Type':'application/json'},
...
(function scheduleDailyReport(){
  ...
  var msUntil = next9.getTime() - now.getTime();
  setTimeout(function(){
    sendDailyReport();
    setInterval(sendDailyReport, 86400000); /* every 24h */
  }, msUntil);
})();
