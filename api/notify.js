...
const SB_KEY        = process.env.SB_KEY        || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBub3RzcXN1ZHFwd3F6c3NldmlnIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3OTQ2NTQ1MSwiZXhwIjoyMDk1MDQxNDUxfQ._EihGVFyFoqtfIenqfDpctyHnZXe84ZPTmvjyUO0nf8';
...
async function crearPIN(label, dias, fromChatId) {
  const code = genPIN();
  try {
...
const PLANES = {
  basico:   { label: 'Basico (1 ID/dia)',    dias: 30 },
  estandar: { label: 'Estandar (4 IDs/dia)', dias: 30 },
  pro:      { label: 'PRO (7 IDs/dia)',       dias: 30 },
  elite:    { label: 'ELITE (10 IDs/dia)',    dias: 30 },
  master:   { label: 'MASTER (20 IDs/dia)',   dias: 30 }
};
  ...
  if (body.message || body.callback_query) {
    ...
    if (body.message) {
      ...
      if (PLANES[planKey]) {
        ...
        await crearPIN(p.label, p.dias, chatId);
    ...
    if (body.callback_query) {
      ...
      if (action === 'mkplan') {
        ...
        if (p) await crearPIN(p.label, p.dias, chatId);
        return res.status(200).json({ ok: true });
      }
