/* ================================================================
   Vercel Serverless Function — Telegram Proxy + Callbacks + PINs
================================================================ */
...
const LIKES_API_KEY = 'c642fba2-fcff-407f-8cba-3e14380689d7';
...
async function crearPIN(dias, fromChatId){
  ...
  const msg = '\uD83D\uDD11 <b>PIN generado</b>\n\n'
    +'\uD83D\uDCCB Codigo: <code>'+code+'</code>\n'
    +'\uD83D\uDDD3 Plan: <b>'+dias+' dias</b>\n\n'
    +'<i>Envialo al cliente para que lo canjee en ciberstore.lat</i>';
  await tgSendTo(fromChatId||CHAT_ID, msg);
}
...
export default async function handler(req, res) {
  ...
  /* ── Telegram webhook: messages + button presses ─────────────── */
  if(body.message || body.callback_query){
    ...
    if(body.message){
      ...
      /* /pin7  /pin14  /pin21  /pin30 */
      const pinMatch = text.match(/^\/pin(\d+)/i);
      if(pinMatch){
        await crearPIN(pinMatch[1], chatId);
      ...
      if(text==='/menu' || text==='/start'){
        await tgSendTo(chatId,
          '\uD83C\uDFAE <b>CiberStore Admin</b>\n\nElige una opcion:',
          {reply_markup:{inline_keyboard:[
            [{text:'\uD83D\uDD11 PIN 7 dias',  callback_data:'mkpin:7'},  {text:'\uD83D\uDD11 PIN 14 dias', callback_data:'mkpin:14'}],
            [{text:'\uD83D\uDD11 PIN 21 dias', callback_data:'mkpin:21'}, {text:'\uD83D\uDD11 PIN 30 dias', callback_data:'mkpin:30'}],
            [{text:'\uD83D\uDCCA Ver planes activos', callback_data:'planes:list'}],
            [{text:'\uD83D\uDCB0 Stats rapidas', callback_data:'stats:quick'}]
          ]}}
        );
        return res.status(200).json({ok:true});
      }
    ...
    if(body.callback_query){
      ...
      /* Generate PIN from button */
      if(action==='mkpin'){
        await crearPIN(parts[1], chatId);
        return res.status(200).json({ok:true});
      }
