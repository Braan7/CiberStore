...
function enviarLikesPanel(){
  ...
  /* Check Supabase historial for this ID sent today */
  ...
  sb.get('likes_historial',
    ...
    _doEnviarLikes(ffId, showS, btn, t0, epd2);
  }).catch(function(){
...
function _doEnviarLikes(ffId, showS, btn, t0, epd2){
  var t0=Date.now();
  ...
  fetch('/api/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'sendlikes',playerId:ffId,username:authSession.username})})
    ...
    .then(function(data){
      ...
      if(!enviadas||parseInt(enviadas)===0||nombre==='Unknown'||nombre==='-'){
        ...
        /* Restore disponibles since send failed */
        window._activePlanDisp=(window._activePlanDisp||0)+1;
        return;
      }
      ...
      sb.get('likes_planes','id=eq.'+window._activePlanId+'&limit=1').then(function(rows){
        ...
        sb.patch('likes_planes',{dias_restantes:nd,likes_enviados:(p.likes_enviados||0)+parseInt(enviadas||0),ultimo_envio:new Date().toISOString(),activo:nd>0,envios_hoy:(p.envios_hoy||0)+1,ff_id:ffId},'id=eq.'+window._activePlanId);
