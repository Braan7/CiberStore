...
function hashPass(pass){
  /* Simple but consistent across sessions */
  var hash = 0;
  var str  = 'cs_' + pass + '_ff';
  for(var i=0; i<str.length; i++){
    var c = str.charCodeAt(i);
    hash  = ((hash << 5) - hash) + c;
    hash  = hash & hash; /* Keep as 32bit */
  }
  /* Convert to unsigned and to base36 string */
  return (hash >>> 0).toString(36) + str.length.toString(16);
}
...
function authTab(tab){
  ...
  for(var i=0;i<panels.length;i++){
    var pel=document.getElementById(panels[i]);
    var tel=document.getElementById(tabIds[i]);
    if(pel) pel.style.display='none';
    if(tel) tel.className='auth-tab';
  }
  var showEl=document.getElementById('auth-'+tab);
  var showTab=document.getElementById('atab-'+tab);
  if(showEl) showEl.style.display='block';
  if(showTab) showTab.className='auth-tab active';
...
function doLogin(){
  ...
  var users = getUsers();
  if(!users[user]){
    err.textContent='Usuario no encontrado. Verifca tu nombre de usuario.';
    err.style.display='block';
    return;
  }
  var storedHash = users[user].passHash;
  var newHash    = hashPass(pass);
  /* If old hash version, update it on successful login attempt */
  if(storedHash !== newHash){
    /* Try old hash format for migration */
    var oldHash = 0;
    for(var oi=0;oi<pass.length;oi++){
      var ochr=pass.charCodeAt(oi);
      oldHash=((oldHash<<5)-oldHash)+ochr;
      oldHash|=0;
    }
    var oldHashStr = oldHash.toString(36);
    if(storedHash === oldHashStr){
      /* Migrate to new hash */
      users[user].passHash = newHash;
      users[user].hashVer  = 2;
      saveUsers(users);
      loginUser(user);
      return;
    }
...
function loginUser(username){
  ...
  /* Ensure user record has all fields */
  var users = getUsers();
  if(users[username]){
    if(!users[username].history)  users[username].history  = [];
    if(!users[username].orders)   users[username].orders   = 0;
    if(!users[username].spent)    users[username].spent    = 0;
    if(!users[username].refCode){
      var chars='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
      var code='REF-';
      for(var i=0;i<6;i++) code+=chars[Math.floor(Math.random()*chars.length)];
      users[username].refCode = code;
    }
    saveUsers(users);
  }
  hideAuthModal();
...
/* Also register VUELVETE5 as a permanent promo if not exists */
(function(){
  var codes=getPromoCodes();
  var found=false;
  for(var i=0;i<codes.length;i++) if(codes[i].code==='VUELVETE5') found=true;
  if(!found){
    codes.push({code:'VUELVETE5',disc:5,maxUses:9999,uses:0,
      desc:'Codigo de salida',active:true,
      created:new Date().toLocaleDateString('es-MX')});
    savePromoCodes(codes);
  }
})();
...
================================================================ */
var RULETA_PRIZES = [
  {label:'2%',  disc:2,  color:'#0055cc'},
  {label:'5%',  disc:5,  color:'#00aaff'},
  {label:'3%',  disc:3,  color:'#7c3aed'},
  {label:'10%', disc:10, color:'#ffd000'},
  {label:'1%',  disc:1,  color:'#333855'},
  {label:'7%',  disc:7,  color:'#00f5ff'},
  {label:'15%', disc:15, color:'#ff4422'},
  {label:'4%',  disc:4,  color:'#25d366'}
];
...
function drawRuleta(angle){
  ...
  ctx.clearRect(0,0,canvas.width,canvas.height);
  for(var i=0;i<n;i++){
    var start = angle + arc*i;
    var end   = start + arc;
    ctx.beginPath();
    ctx.moveTo(cx,cy);
    ctx.arc(cx,cy,r,start,end);
    ctx.closePath();
    ctx.fillStyle = RULETA_PRIZES[i].color;
    ctx.fill();
    ...
    ctx.fillText(RULETA_PRIZES[i].label, r-8, 5);
    ctx.restore();
  }
  /* Center circle */
  ctx.beginPath();
  ...
  ctx.fill();
  ctx.strokeStyle = 'rgba(0,170,255,.5)';
  ctx.lineWidth = 2;
  ctx.stroke();
}

function openRuleta(){
  if(!authSession){ showToast('Inicia sesion para girar la ruleta'); setTimeout(showAuthModal,600); return; }
  ...
  var result   = document.getElementById('ruleta-result');
  var nextEl   = document.getElementById('ruleta-next');
  var btn      = document.getElementById('ruleta-btn');
  var now      = Date.now();
  var canSpin  = !lastSpin || (now - parseInt(lastSpin)) > 86400000;
  if(result) result.textContent='';
  if(canSpin){
    if(btn){ btn.disabled=false; btn.style.opacity='1'; btn.textContent='GIRAR \uD83C\uDFB2'; }
    if(nextEl) nextEl.style.display='none';
  } else {
    var remaining = 86400000 - (now - parseInt(lastSpin));
    var h=Math.floor(remaining/3600000);
    var m=Math.floor((remaining%3600000)/60000);
    if(btn){ btn.disabled=true; btn.style.opacity='.45'; btn.textContent='Ya giraste hoy'; }
    if(nextEl){ nextEl.style.display='block'; nextEl.textContent='Vuelve en '+h+'h '+m+'m'; }
    /* Show last prize */
    var lastPrize = localStorage.getItem('cs_ruleta_prize');
    if(lastPrize && result) result.textContent='Tu ultimo premio: '+lastPrize+'% de descuento!';
  }
}
...
function spinRuleta(){
  ...
  function animate(ts){
    ...
    if(progress < 1){
      ...
      if(!found){
        codes.push({code:code,disc:prize.disc,maxUses:1,uses:0,
          desc:'Premio ruleta',active:true,
          created:new Date().toLocaleDateString('es-MX')});
        savePromoCodes(codes);
      }
      if(btn){ btn.textContent='Usar codigo: '+code; btn.disabled=false;
        btn.onclick=function(){ closeRuleta(); var inp=document.getElementById('f-promo'); if(inp) inp.value=code; showToast('Codigo '+code+' aplicado! Ve a comprar.', 3000); }; }
      launchConfetti();
      showToast('Ganaste '+prize.disc+'% de descuento!', 3000);
    }
  }
  requestAnimationFrame(animate);
}
(function(){
  var el=document.getElementById('modal-ruleta');
  if(el) el.addEventListener('click',function(e){ if(e.target===this) closeRuleta(); });
})();
...
function requestNotifPerm(){
  ...
  Notification.requestPermission().then(function(perm){
    ...
    if(perm==='granted'){
      ...
      sendNotif('CiberStore', 'Notificaciones activadas! Te avisaremos de ofertas y novedades.');
      showToast('Notificaciones activadas!', 2500);
    } else {
      showToast('Permiso denegado. Activalas en ajustes del navegador.');
    }
  });
}
...
function getChatMsgs(){
  try{ return JSON.parse(localStorage.getItem('cs_chat')||'[]'); }
  catch(e){ return []; }
}
function saveChatMsgs(m){ localStorage.setItem('cs_chat', JSON.stringify(m)); }
...
function likeMsg(id){
  ...
  var liked = JSON.parse(localStorage.getItem('cs_chat_liked')||'[]');
  if(liked.indexOf(id)>=0){ showToast('Ya le diste like'); return; }
  liked.push(id);
  localStorage.setItem('cs_chat_liked', JSON.stringify(liked));
  var msgs = getChatMsgs();
  for(var i=0;i<msgs.length;i++) if(msgs[i].id===id){ msgs[i].likes=(msgs[i].likes||0)+1; break; }
  saveChatMsgs(msgs);
  renderChat();
}

function deleteChatMsg(id){
  var msgs = getChatMsgs().filter(function(m){ return m.id!==id; });
  saveChatMsgs(msgs);
  renderChat();
}
...
function renderChat(){
  ...
  for(var i=0;i<msgs.length;i++){
    var m=msgs[i];
    var isMe = authSession && m.user===authSession.username;
    var isAdmin2 = adminAuthed;
    rows+='<div style="background:var(--card);border:1px solid var(--border);border-radius:11px;padding:.85rem">'
      +'<div style="display:flex;align-items:center;gap:.55rem;margin-bottom:.45rem">'
      +'<div style="width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,var(--c2),var(--c1));display:flex;align-items:center;justify-content:center;font-family:Orbitron;font-size:.6rem;font-weight:700;color:#fff;flex-shrink:0">'+m.user.charAt(0).toUpperCase()+'</div>'
      +'<div style="flex:1"><span style="font-size:.78rem;font-weight:700;color:#fff">'+m.user+'</span>'
      +(isMe?' <span style="font-size:.58rem;color:var(--c1);font-weight:600">Tu</span>':'')+'</div>'
      +'<span style="font-size:.62rem;color:var(--muted)">'+m.fecha+'</span>'
      +'</div>'
      +'<div style="font-size:.82rem;color:var(--text);line-height:1.6;margin-bottom:.5rem">'+m.text+'</div>'
      ...
      +((isMe||isAdmin2)?'<button data-id="'+m.id+'" onclick="deleteChatMsg(+this.dataset.id)" style="background:none;border:1px solid rgba(255,80,80,.2);color:rgba(255,80,80,.6);border-radius:5px;padding:.18rem .5rem;font-size:.68rem;cursor:pointer">Eliminar</button>':'')
      +'</div>'
      +'</div>';
  }
  feed.innerHTML=rows;
...
/* ================================================================
   DASHBOARD DE VENTAS + EXPORT CSV
================================================================ */
function renderSalesDash(){
  ...
  for(var d=6;d>=0;d--){
    ...
    days.push(label);
    /* Sum up history for this date */
    var total=0;
    for(var i=0;i<uKeys.length;i++){
      var hist=(users[uKeys[i]].history)||[];
      for(var j=0;j<hist.length;j++){
        if(hist[j].date && hist[j].date.indexOf(label)>=0) total+=hist[j].price||0;
      }
    }
    sales.push(total);
  }
  ...
  var pad=24;
  var barW=Math.floor((W-pad*2)/7)-4;
  ...
  for(var k=0;k<7;k++){
    ...
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(x,H-barH-10,barW,barH,3);
    else ctx.rect(x,H-barH-10,barW,barH);
    ctx.fill();
    ctx.fillStyle='rgba(90,101,128,.8)';
    ctx.font='9px Exo 2,sans-serif';
    ctx.textAlign='center';
    ctx.fillText(days[k],x+barW/2,H);
    if(sales[k]>0){
      ctx.fillStyle='#fff';
      ctx.font='bold 9px Exo 2,sans-serif';
      ctx.fillText('$'+sales[k],x+barW/2,H-barH-12);
    }
  }
  ...
  if(extra){
    var totalUsers=uKeys.length;
    var totalSpent2=uKeys.reduce(function(s,k2){ return s+(users[k2].spent||0); },0);
    var totalOrds=uKeys.reduce(function(s,k2){ return s+(users[k2].orders||0); },0);
    var avgTicket=totalOrds>0?Math.round(totalSpent2/totalOrds):0;
    extra.innerHTML=
      ...
      +'<div style="font-family:Orbitron;font-size:.9rem;font-weight:700;color:var(--c1)">$'+avgTicket.toLocaleString('es-MX')+'</div>'
      ...
      +'<div style="font-family:Orbitron;font-size:.9rem;font-weight:700;color:var(--c4)">'+totalUsers+'</div>'
      +'<div style="font-size:.6rem;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-top:.1rem">Clientes registrados</div></div>';
  }
}
...
function exportCSV(){
  var users=getUsers();
  var uKeys=Object.keys(users);
  var rows=[['Usuario','Fecha registro','Pedidos','Gastado MXN','Nivel','Codigo referido']];
  for(var i=0;i<uKeys.length;i++){
    var u=users[uKeys[i]];
    var tier=getTier?getTier(u.spent||0):{name:'Visitante'};
    rows.push([uKeys[i],u.created||'',u.orders||0,u.spent||0,tier.name,u.refCode||'']);
  }
  var csv=rows.map(function(r){ return r.map(function(c){ return '"'+String(c).replace(/"/g,'""')+'"'; }).join(','); }).join('\n');
  var blob=new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8;'});
  var url=URL.createObjectURL(blob);
  var a=document.createElement('a');
  a.href=url; a.download='ciberstore_usuarios_'+new Date().toISOString().slice(0,10)+'.csv';
  a.click();
  URL.revokeObjectURL(url);
  showToast('CSV descargado!', 2000);
}

/* Patch admSubTab to render dash */
var _origAdmSubTab=admSubTab;
admSubTab=function(tab){
  _origAdmSubTab(tab);
  if(tab==='dash'){
    var dl=document.getElementById('adm-tab-dash');
    if(dl) dl.style.display='block';
    setTimeout(renderSalesDash, 80);
    /* Active style for dash btn */
    var db=document.getElementById('adm-stab-dash');
    if(db){ db.style.background='rgba(124,58,237,.2)'; db.style.borderColor='rgba(124,58,237,.45)'; db.style.color='#a78bfa'; }
  }
};

/* Patch goPage for comunidad */
var _origGoPageComun=goPage;
goPage=function(id){
  _origGoPageComun(id);
