...
function loadRanking(){
  ...
  sb.get('profiles','select=id,username,role').then(function(profs){
    ...
    sb.get('movimientos_saldo','tipo=eq.credito&created_at=gte.'+since+'&select=user_id,monto')
      .then(function(movs){
        ...
        var sorted = Object.keys(agg).map(function(uid){
          return {uid:uid, username:umap[uid]||'Usuario', total:agg[uid].total, count:agg[uid].count};
    ...
    /* Top Gastos (compras) */
    sb.get('movimientos_saldo','tipo=eq.compra&created_at=gte.'+since+'&select=user_id,monto')
      .then(function(movs){
        var agg={};
        if(movs) movs.forEach(function(m){
...
goPage = function(id){
  _origGoPageTf(id);
  if(id==='perfil') setTimeout(initTransfer, 300);
};
  ...
  sb.get('movimientos_saldo','tipo=eq.compra&select=user_id,descripcion,monto')
    .then(function(movs){
      ...
      movs.forEach(function(m){
        ...
        if(d.indexOf('like')>=0){
          var matchL = d.match(/(\d[\d,]*)\s*like/);
          agg[uid].likes += matchL ? parseInt(matchL[1].replace(/,/g,''))||0 : 1000;
        }
      });
