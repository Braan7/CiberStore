...
var PRODUCTS = [
  {id:1,  name:'100',    total:100,   bonus:10,  region:'LATAM & BR', prices:[17,17,17,17,17],       badge:null,        isPase:false, popular:false},
  {id:2,  name:'310',    total:310,   bonus:31,  region:'LATAM & BR', prices:[60,60,60,60,60],        badge:'POPULAR',   isPase:false, popular:true},
  {id:3,  name:'520',    total:520,   bonus:52,  region:'LATAM & BR', prices:[85,85,85,85,85],        badge:null,        isPase:false, popular:false},
  {id:4,  name:'1,060',  total:1060,  bonus:106, region:'LATAM & BR', prices:[175,173,170,168,166],   badge:'OFERTA',    isPase:false, popular:false},
  {id:5,  name:'2,180',  total:2180,  bonus:218, region:'LATAM & BR', prices:[310,306,302,298,294],   badge:null,        isPase:false, popular:false},
  {id:6,  name:'5,600',  total:5600,  bonus:560, region:'LATAM & BR', prices:[770,760,750,740,730],   badge:'GRAN VALOR',isPase:false, popular:false},
  {id:7,  name:'11,200', total:11200, bonus:1120,region:'LATAM & BR', prices:[1540,1520,1500,1480,1460],badge:null,      isPase:false, popular:false},
  {id:8,  name:'16,800', total:16800, bonus:1680,region:'LATAM & BR', prices:[2310,2280,2250,2220,2190],badge:'MEGA',    isPase:false, popular:false},
...
function getNextOrder(){
  /* Generate unique order: timestamp base36 + random */
  var ts  = Date.now().toString(36).toUpperCase().slice(-4);
  var rnd = Math.floor(Math.random()*9000+1000).toString();
  var ord = parseInt(rnd + ts, 36) % 90000 + 10000;
  /* Also store last order for reference */
  localStorage.setItem('cs_last_ord', ord);
  return ord;
}
function peekOrder(){
  var ts  = Date.now().toString(36).toUpperCase().slice(-4);
  var rnd = Math.floor(Math.random()*9000+1000).toString();
  return parseInt(rnd + ts, 36) % 90000 + 10000;
}
...
function doAdminLogin(){
  ...
  if(!emailOk || !passOk){
    ...
    if(!emailOk) msg += ' (email: ' + email + ')';
    if(!passOk)  msg += ' (pass length: ' + pass.length + ')';
...
function renderProds(){
  ...
  for(var i=0;i<PRODUCTS.length;i++){
    ...
    var bonusHtml=p.bonus>0?'<div class="ff-bonus">+'+p.bonus+' BONUS incluido</div>':'';
...
/* ================================================================
   LEADERBOARD \u2014 Top Compradores
================================================================ */
function loadLeaderboard(){
  var list = document.getElementById('leaderboard-list');
  if(!list) return;
  list.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--muted);font-size:.82rem">Cargando...</div>';

  /* Get top users by spent from Supabase */
  sb.get('profiles', 'select=username,saldo&order=saldo.desc&limit=10')
    .then(function(users){
      if(!users || !Array.isArray(users) || !users.length){
        list.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--muted);font-size:.82rem">Sin datos aun</div>';
        return;
      }
      /* Filter users with saldo > 0 */
      var top = users.filter(function(u){ return (u.saldo||0) > 0; });
      if(!top.length){
        /* Show demo data if no real data yet */
        top = [
          {username:'ciberstore', saldo:5000},
          {username:'jugador_ff', saldo:3200},
          {username:'pro_gamer', saldo:1800}
        ];
      }
      ...
      for(var i=0; i<Math.min(top.length, 10); i++){
        ...
        var color = i < 3 ? colors[i] : '#5a6580';
        var isFirst = i === 0;
        var saldo = (u.saldo||0).toLocaleString('es-MX');
        var isMe  = authSession && authSession.username === u.username;

        h += '<div style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1.25rem;'
          + (isFirst ? 'background:rgba(255,215,0,.05);' : '')
          + (isMe    ? 'background:rgba(0,170,255,.06);' : '')
          + 'border-bottom:1px solid rgba(255,255,255,.04)">'
          /* Position */
          + '<div style="width:32px;text-align:center;font-size:'+(i<3?'1.2rem':'.85rem')+';'
          + 'font-family:Orbitron;font-weight:700;color:'+color+';flex-shrink:0">'
          + medal + '</div>'
          /* Avatar */
          + '<div style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,var(--c2),var(--c1));'
          + 'display:flex;align-items:center;justify-content:center;font-family:Orbitron;font-size:.75rem;'
          + 'font-weight:700;color:#fff;flex-shrink:0;border:2px solid '+(isFirst?'#ffd700':color)+'44">'
          ...
          + u.username + (isMe ? ' <span style="font-size:.6rem;color:var(--c1)">(Tu)</span>' : '')
          + '</div>'
          + '<div style="font-size:.62rem;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Cliente CiberStore</div>'
          + '</div>'
          /* Amount */
          + '<div style="text-align:right;flex-shrink:0">'
          + '<div style="font-family:Orbitron;font-size:.88rem;font-weight:900;color:'+(isFirst?'#ffd700':'#00e676')+'">'
          + '$' + saldo + '</div>'
          + '<div style="font-size:.58rem;color:var(--muted);text-transform:uppercase;letter-spacing:1px">MX gastado</div>'
          + '</div>'
          + '</div>';
      }

      list.innerHTML = h || '<div style="text-align:center;padding:1.5rem;color:var(--muted)">Sin datos</div>';
    })
    .catch(function(){
      list.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--muted);font-size:.82rem">Error al cargar</div>';
    });
}

/* Load leaderboard on home page visit */
...
/* After admin logs in, open full panel */
var _origDoAdminLogin2 = doAdminLogin;
...
function admFullTab(tab){
  var tabs = ['stats','users','saldos','codigos','chat','resenas'];
...
function admFullLoadStats(){
  sb.get('profiles','select=saldo,role,banned').then(function(users){
    ...
    var totalAdmins = users.filter(function(u){return u.role==='admin';}).length;
    ...
    if(el2) el2.textContent = '$'+totalSaldo.toLocaleString('es-MX');
    if(el3) el3.textContent = totalAdmins;
    /* Also update old stats if visible */
    renderAdminStats2(users);
  }).catch(function(){});
  /* Movimientos count */
  sb.get('movimientos_saldo','select=id').then(function(movs){
    var el4=document.getElementById('adm-s-movs');
    if(el4&&movs) el4.textContent = movs.length;
  }).catch(function(){});
  /* Sales chart */
  var canvas=document.getElementById('adm-sales-chart');
  if(canvas) renderSalesDashOnCanvas(canvas,'adm-chart-extra');
}

function renderSalesDashOnCanvas(canvas, extraId){
  ...
  sb.get('movimientos_saldo','tipo=eq.compra&created_at=gte.'+since+'&order=created_at.asc')
    .then(function(rows){
      ...
      for(var k=0;k<7;k++){
        ...
        if(sales[k]>0){ctx.fillStyle='#fff';ctx.font='bold 9px sans-serif';ctx.fillText('$'+sales[k],x+bw/2,H-bh-13);}
      }
    }).catch(function(){});
}
...
function admFullLoadUsers(){
  ...
  sb.get('profiles',qs).then(function(users){
    if(!users||!Array.isArray(users)||!users.length){
      tbody.innerHTML='<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:2rem">Sin usuarios'+(q?' con ese nombre':'')+'</td></tr>';
      return;
    }
    var rows='';
    users.forEach(function(u){
      ...
      rows+='<tr>'
        ...
        +'<button data-u="'+u.id+'" data-n="'+u.username+'" onclick="admAddSaldo(this.dataset.u,this.dataset.n)" class="adm-action-btn" style="border-color:rgba(0,230,118,.3);color:#00e676">+Saldo</button>'
        +'<button data-u="'+u.id+'" data-n="'+u.username+'" onclick="admQuitarSaldo(this.dataset.u,this.dataset.n)" class="adm-action-btn" style="border-color:rgba(255,80,80,.3);color:#ff6b6b">-Saldo</button>'
        +'<button data-u="'+u.id+'" data-n="'+u.username+'" onclick="admVerHistorial(this.dataset.u,this.dataset.n)" class="adm-action-btn" style="border-color:rgba(0,170,255,.3);color:var(--c1)">Historial</button>'
        ...
        +'<button data-u="'+u.id+'" data-n="'+u.username+'" data-b="'+u.banned+'" onclick="admBanear(this.dataset.u,this.dataset.n,this.dataset.b)" class="adm-action-btn" style="border-color:rgba(255,165,0,.3);color:#ffa500">'+(u.banned?'Desbanear':'Banear')+'</button>'
  ...
  sb.get('movimientos_saldo','order=created_at.desc&limit=50').then(function(movs){
    ...
    sb.get('profiles','select=id,username').then(function(profs){
      ...
      movs.forEach(function(m){
        ...
        var isC=m.tipo==='credito'||m.tipo==='ajuste';
        rows+='<tr>'
          +'<td style="color:#fff;font-weight:600">'+(umap[m.user_id]||m.user_id||'-')+'</td>'
...
function admSaldoAction(action){
  ...
  if(!username){if(msgEl){msgEl.textContent='Ingresa el usuario';msgEl.style.color='#ff6b6b';msgEl.style.display='block';}return;}
  if(!monto||monto<=0){if(msgEl){msgEl.textContent='Ingresa un monto valido';msgEl.style.color='#ff6b6b';msgEl.style.display='block';}return;}
  sb.get('profiles','username=eq.'+encodeURIComponent(username)+'&limit=1').then(function(users){
    if(!users||!users[0]){if(msgEl){msgEl.textContent='Usuario no encontrado';msgEl.style.color='#ff6b6b';msgEl.style.display='block';}return;}
    var u=users[0];
    ...
    sbUpdateProfile(u.id,{saldo:newSaldo}).then(function(){
      ...
      if(msgEl){
        msgEl.textContent=(action==='add'?'+':'-')+'$'+monto+' MX a '+username+'. Nuevo saldo: $'+newSaldo.toLocaleString('es-MX');
        msgEl.style.color=action==='add'?'#00e676':'#ff6b6b';
        msgEl.style.display='block';
      }
      admFullLoadMovs();
    });
  }).catch(function(){if(msgEl){msgEl.textContent='Error de conexion';msgEl.style.color='#ff6b6b';msgEl.style.display='block';}});
}
