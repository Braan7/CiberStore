...
var LIKES = [
  {id:1,  label:'7 dias',      emoji:'\uD83D\uDD25', priceMX:20,   priceUSD:1.0,  total:1540,  perDay:220,  days:7,   color:'#cd7f32', popular:false, best:false},
  {id:2,  label:'14 dias',     emoji:'\uD83D\uDD25', priceMX:35,   priceUSD:1.8,  total:3080,  perDay:220,  days:14,  color:'#c0c0c0', popular:true,  best:false},
  {id:3,  label:'21 dias',     emoji:'\uD83D\uDD25', priceMX:50,   priceUSD:2.6,  total:4620,  perDay:220,  days:21,  color:'#ffd700', popular:false, best:false},
  {id:4,  label:'30 dias',     emoji:'\uD83D\uDD25', priceMX:65,   priceUSD:3.3,  total:6600,  perDay:220,  days:30,  color:'#00aaff', popular:false, best:true},
  {id:10, label:'Instantaneos',emoji:'\u26A1',        priceMX:165,  priceUSD:8.4,  total:2200,  perDay:2200, days:1,   color:'#00f5ff', popular:false, best:false, isInstant:true}
];

var HONOR = [
  {region:'Norteamerica',flag:'\uD83C\uDDE8\uD83C\uDDE6',color:'#ffd000',price:340},
  {region:'Estados Unidos',flag:'\uD83C\uDDFA\uD83C\uDDF8',color:'#4dabf7',price:245},
...
function openFragModal(frags, diam, usd, mxn){
  ...
  var sd  = document.getElementById('frag-saldo-disp');
  ...
  if(sd)  sd.textContent  = '$' + ((authSession&&authSession.saldo)||0).toLocaleString('es-MX') + ' MX';
  if(err) err.style.display = 'none';
  if(fi)  fi.value = '';
  if(fn)  fn.value = '';
...
function submitFrag(){
  ...
  if(!authSession){ showErr('Inicia sesion para comprar'); return; }
  if(!ffId)  { showErr('Ingresa tu ID de Free Fire'); return; }
  if(!ffNom) { showErr('Ingresa tu nombre en el juego'); return; }

  var saldo = authSession.saldo||0;
  if(saldo < fragCurrent.mxn){
    showErr('Saldo insuficiente ($'+saldo.toLocaleString('es-MX')+' MX). Recarga tu cuenta.');
    return;
  }

  var ord = getNextOrder();
  addSpend(fragCurrent.mxn, 'Fragmentos Evo '+fragCurrent.frags+' frags - Pedido #'+ord);
  if(typeof tgNotifyPurchase==='function') tgNotifyPurchase(authSession.username, 'Fragmentos Evo '+fragCurrent.frags, fragCurrent.mxn, ord);
...
/* ================================================================
   SISTEMA DE PINs \u2014 Membresias de Likes
================================================================ */

function activarPIN(){
  if(!authSession){ showToast('Inicia sesion primero'); setTimeout(showAuthModal,500); return; }
  var ffId = ((document.getElementById('pin-ff-id')||{}).value||'').trim();
  ...
  if(!ffId){ showMsg('Ingresa tu ID de Free Fire', false); return; }
  if(!pin || pin.length < 4){ showMsg('Ingresa un PIN valido', false); return; }

  showMsg('Verificando PIN...', true);

  /* Check PIN in Supabase */
  sb.get('likes_pines',
    'codigo=eq.'+encodeURIComponent(pin)+'&activo=eq.true&usado=eq.false&limit=1'
  ).then(function(rows){
    if(!rows||!rows.length){
      showMsg('\u274C PIN invalido o ya fue utilizado', false);
      return;
    }
    var pinData = rows[0];
    ...
    sb.get('likes_planes',
      'user_id=eq.'+authSession.id+'&activo=eq.true&limit=1'
    ).then(function(planes){
      if(planes&&planes.length){
        showMsg('\u26A0\uFE0F Ya tienes un plan activo. Espera a que termine.', false);
        return;
      }
      ...
      /* Create active plan */
      sb.post('likes_planes', {
        user_id:        authSession.id,
        username:       authSession.username,
        ff_id:          ffId,
        plan_dias:      pinData.dias,
        dias_restantes: pinData.dias,
        likes_enviados: 0,
        activo:         true
      }).then(function(){
        showMsg('\u2705 PIN activado! Plan de '+pinData.dias+' dias activo.', true);
        /* Show active plan UI */
        var ap = document.getElementById('pin-active-plan');
        var pn = document.getElementById('pin-plan-name');
        var dr = document.getElementById('pin-dias-rest');
        if(ap) ap.style.display='block';
        if(pn) pn.textContent = pinData.dias+' dias de likes';
        if(dr) dr.textContent = pinData.dias;
        /* Notify Telegram */
        if(typeof tgSend==='function'){
          tgSend('\uD83D\uDC4D <b>PIN activado - CiberStore</b>\n\n\uD83D\uDC64 Usuario: <b>'+authSession.username+'</b>\n\uD83C\uDFAE ID FF: '+ffId+'\n\uD83D\uDDD3 Plan: '+pinData.dias+' dias\n\uD83D\uDD11 PIN: '+pin);
        }
        loadLikesHistory();
        /* Clear fields */
        var pf=document.getElementById('pin-ff-id');
        var pc=document.getElementById('pin-code');
        if(pf) pf.value=''; if(pc) pc.value='';
      }).catch(function(e){ showMsg('Error al activar: '+(e.message||''), false); });
    }).catch(function(){ showMsg('Error al verificar plan', false); });
  }).catch(function(){ showMsg('Error al verificar PIN', false); });
}

function enviarLikesConPlan(){
  if(!authSession){ showToast('Inicia sesion primero'); return; }
  var ffId = ((document.getElementById('pin-send-id')||{}).value||'').trim();
  var msg  = document.getElementById('pin-send-msg');
  ...
  if(!ffId){ showMsg('Ingresa tu UID de Free Fire', false); return; }

  /* Get active plan */
  sb.get('likes_planes','user_id=eq.'+authSession.id+'&activo=eq.true&limit=1')
    .then(function(rows){
      if(!rows||!rows.length){ showMsg('No tienes un plan activo', false); return; }
      var plan = rows[0];

      /* Check if already sent today */
      var hoy = new Date().toDateString();
      var lastSend = plan.ultimo_envio ? new Date(plan.ultimo_envio).toDateString() : '';
      if(lastSend === hoy){ showMsg('\u26A0\uFE0F Ya enviaste likes hoy. Vuelve ma\u00F1ana.', false); return; }
      if(plan.dias_restantes <= 0){ showMsg('Tu plan ha terminado', false); return; }

      showMsg('Enviando likes...', true);
      ...
      fetch('/api/notify', {
        ...
        .then(function(data){
          var enviadas = (data.result&&data.result.data&&data.result.data[0]&&data.result.data[0].likes&&data.result.data[0].likes.enviadas)||'~220';
          var depois   = (data.result&&data.result.data&&data.result.data[0]&&data.result.data[0].likes&&data.result.data[0].likes.depois)||'?';

          /* Update plan in Supabase */
          var nuevosDias   = plan.dias_restantes - 1;
          var nuevosLikes  = (plan.likes_enviados||0) + parseInt(enviadas||220);
          var updates = {
            dias_restantes: nuevosDias,
            likes_enviados: nuevosLikes,
            ultimo_envio:   new Date().toISOString(),
            activo:         nuevosDias > 0
          };
          sb.patch('likes_planes', updates, 'id=eq.'+plan.id);
          ...
          sb.post('likes_historial', {
            ...
            antes:     parseInt(depois||0) - parseInt(enviadas||0),
            despues:   parseInt(depois||0)
          }).catch(function(){});

          showMsg('\u2705 +'+enviadas+' likes enviados! Total ahora: '+depois, true);
          /* Update UI */
          var dr=document.getElementById('pin-dias-rest');
          var lt=document.getElementById('pin-likes-total');
          var di=document.getElementById('pin-disponibles');
          if(dr) dr.textContent = nuevosDias;
          if(lt) lt.textContent = nuevosLikes.toLocaleString('es-MX');
          if(di) di.textContent = '0';
          /* Clear input */
          var si=document.getElementById('pin-send-id');
          if(si) si.value='';
          loadLikesHistory();
        }).catch(function(e){
          showMsg('Error al enviar: '+(e.message||''), false);
        });
    }).catch(function(){ showMsg('Error al verificar plan', false); });
}
...
function loadLikesHistory(){
  ...
  sb.get('likes_planes','user_id=eq.'+authSession.id+'&order=created_at.desc&limit=10')
    .then(function(rows){
      if(!rows||!rows.length){
        el.innerHTML='<div style="text-align:center;padding:1.5rem;color:var(--muted);font-size:.82rem">Sin historial de membres\u00EDas aun</div>';
        return;
      }
      var h='';
      rows.forEach(function(r){
        var fecha = r.created_at ? new Date(r.created_at).toLocaleDateString('es-MX',{day:'2-digit',month:'short',year:'numeric'}) : '';
        var isActive = r.activo && r.dias_restantes > 0;
        h+='<div style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;border-bottom:1px solid rgba(255,255,255,.04)">'
          +'<div style="width:36px;height:36px;border-radius:9px;background:rgba('+(isActive?'0,230,118':'255,255,255')+',.06);border:1px solid rgba('+(isActive?'0,230,118':'255,255,255')+',.12);display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0">'+(isActive?'\uD83D\uDFE2':'\u26AB')+'</div>'
          +'<div style="flex:1;min-width:0">'
            +'<div style="font-size:.82rem;font-weight:600;color:#fff">'
              +(isActive?'<span style="font-size:.65rem;background:rgba(0,230,118,.15);color:#00e676;padding:.1rem .4rem;border-radius:4px;margin-right:.4rem;font-family:\'Exo 2\'">ACTIVO</span>':'')
              +'Plan '+r.plan_dias+' dias'
            +'</div>'
            +'<div style="font-size:.7rem;color:var(--muted);margin-top:.15rem">ID: '+r.ff_id+' &bull; Activado: '+fecha+'</div>'
          +'</div>'
          +'<div style="text-align:right;flex-shrink:0">'
            +'<div style="font-family:Orbitron;font-size:.78rem;font-weight:700;color:'+(isActive?'#00e676':'var(--muted)')+'">'+r.dias_restantes+' dias</div>'
            +'<div style="font-size:.62rem;color:var(--muted)">'+(r.likes_enviados||0)+' likes</div>'
          +'</div>'
          +'</div>';
      });
      el.innerHTML = h;
    }).catch(function(){
      el.innerHTML='<div style="text-align:center;padding:1.5rem;color:#ff6b6b;font-size:.8rem">Error al cargar historial</div>';
    });
}
...
function checkActivePlan(){
  ...
  sb.get('likes_planes','user_id=eq.'+authSession.id+'&activo=eq.true&order=created_at.desc&limit=1')
    .then(function(rows){
      var np = document.getElementById('pin-no-plan');
      var ap = document.getElementById('pin-active-plan');
      var ss = document.getElementById('pin-send-section');
      if(!rows||!rows.length){
      ...
      var pn = document.getElementById('pin-plan-name');
      var ps = document.getElementById('pin-plan-sub');
      var dr = document.getElementById('pin-dias-rest');
      var lt = document.getElementById('pin-likes-total');
      var di = document.getElementById('pin-disponibles');
      if(pn) pn.textContent = 'Plan '+r.plan_dias+' dias';
      if(ps) ps.textContent = r.dias_restantes+' dias restantes \u00B7 220 likes/dia';
      if(dr) dr.textContent = r.dias_restantes;
      if(lt) lt.textContent = (r.likes_enviados||0).toLocaleString('es-MX');
      /* Available sends today - check ultimo_envio */
      var hoy = new Date().toDateString();
      var lastSend = r.ultimo_envio ? new Date(r.ultimo_envio).toDateString() : '';
      var disponibles = (lastSend === hoy) ? 0 : 1;
      if(di) di.textContent = disponibles;
    }).catch(function(){});
}
...
goPage = function(id){
  _origGoPageLikes(id);
  if(id==='likes'){
    setTimeout(checkActivePlan, 300);
  }
};

/* ================================================================
   ADMIN \u2014 Generar PINs de likes
================================================================ */
function admGenerarPIN(dias){
  ...
  sb.post('likes_pines',{
    ...
    activo: true,
    usado:  false,
    creado_por: authSession ? authSession.username : 'admin'
  }).then(function(){
    showToast('\u2705 PIN generado: '+code+' ('+dias+' dias)');
    /* Copy to clipboard */
...
function admLoadPines(){
  ...
  sb.get('likes_pines','order=created_at.desc&limit=30').then(function(rows){
    ...
    var h='<table class="adm-table"><thead><tr><th>Codigo</th><th>Dias</th><th>Estado</th><th>Usado por</th><th>Fecha</th><th>Accion</th></tr></thead><tbody>';
    rows.forEach(function(r){
      var fecha = r.created_at ? new Date(r.created_at).toLocaleDateString('es-MX') : '';
      var estado = r.usado ? '<span class="adm-badge" style="background:rgba(255,80,80,.1);color:#ff6b6b;border:1px solid rgba(255,80,80,.2)">USADO</span>'
                           : '<span class="adm-badge" style="background:rgba(0,230,118,.1);color:#00e676;border:1px solid rgba(0,230,118,.2)">DISPONIBLE</span>';
      h+='<tr>'
        +'<td style="font-family:Orbitron;font-weight:700;color:#fff;letter-spacing:1px">'+r.codigo+'</td>'
        +'<td style="color:var(--c1)">'+r.dias+' dias</td>'
        +'<td>'+estado+'</td>'
        +'<td style="color:var(--muted)">'+(r.usado_por||'-')+'</td>'
        +'<td style="color:var(--muted)">'+fecha+'</td>'
        +'<td>'
          +(r.usado?'':
            '<button data-id="'+r.id+'" data-code="'+r.codigo+'" onclick="admCopyPIN(this.dataset.code)" class="adm-action-btn" style="padding:.3rem .65rem;border-color:rgba(0,170,255,.3);color:var(--c1)">Copiar</button> '
          +'<button data-id="'+r.id+'" onclick="admDeletePIN(this.dataset.id)" class="adm-action-btn" style="padding:.3rem .65rem;border-color:rgba(255,80,80,.3);color:#ff6b6b">Borrar</button>'
          )
        +'</td>'
        +'</tr>';
    });
    h+='</tbody></table>';
    el.innerHTML=h;
  }).catch(function(){ el.innerHTML='<div style="color:#ff6b6b;padding:1rem">Error al cargar</div>'; });
}

function admCopyPIN(code){
  if(navigator.clipboard) navigator.clipboard.writeText(code).then(function(){ showToast('\u2705 Copiado: '+code); });
}

function admDeletePIN(id){
  if(!confirm('Borrar este PIN?')) return;
  sb.del('likes_pines','id=eq.'+id).then(function(){ admLoadPines(); showToast('PIN eliminado'); });
}
